...
async fn run_deploy_script(project_dir: &Path) -> Result<(), String> {
    let script_path = project_dir.join("deploy").join("gcp").join("deploy.sh");
    ...
    let output = Command::new("bash")
        ...
        .output()
        .await
        .map_err(|e| format!("failed to execute gcloud deploy script: {e}"))?;

    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        return Err(format!("gcloud deploy.sh failed: {stderr}"));
    }
    Ok(())
}
