...
pub fn extract_auth_user(
    ...
    jwt_secret: &str,
) -> Result<AuthUser, crate::errors::MailError> {
    let auth_header = req
        .headers()
        .get("Authorization")
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| {
            crate::errors::MailError::Unauthorized("Missing Authorization header".into())
        })?;

    let token = auth_header.strip_prefix("Bearer ").ok_or_else(|| {
        crate::errors::MailError::Unauthorized("Invalid Authorization header format".into())
    })?;
    ...
    .map_err(|e| crate::errors::MailError::Unauthorized(format!("Invalid token: {e}")))?;

    let Ok(user_id) = uuid::Uuid::parse_str(&token_data.claims.sub) else {
        return Err(crate::errors::MailError::Unauthorized(
...
pub fn get_auth_user(req: &actix_web::HttpRequest) -> Result<AuthUser, crate::errors::MailError> {
    ...
    let jwt_secret = req
        ...
        .ok_or_else(|| crate::errors::MailError::Internal("JWT secret not configured".into()))?;
...
pub fn require_admin(user: &AuthUser) -> Result<(), crate::errors::MailError> {
    if user.is_admin() {
        Ok(())
    } else {
        Err(crate::errors::MailError::Forbidden(
            "Admin role required".into(),
        ))
    }
}

pub fn require_admin_or_organizer(user: &AuthUser) -> Result<(), crate::errors::MailError> {
