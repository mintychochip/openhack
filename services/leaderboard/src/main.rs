...
/// Start the Actix-web HTTP server for the leaderboard service.
///
/// # Expected Behavior
///
/// Loads configuration from environment variables, initializes the `PostgreSQL`
/// connection pool and Redis client, runs database health checks, spawns a
/// Redis pub/sub subscriber for recalculation triggers, and starts the
/// Actix-web server on the configured port (default 3004). Graceful
...
/// - Spawns a background task for Redis pub/sub subscriber (async task).
...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("Starting leaderboard service on port {}", config.port);

    openhack_common::metrics::register_business_counter("leaderboard_votes_total", "Total public votes cast");
    openhack_common::metrics::register_business_counter("leaderboard_rankings_updated_total", "Total ranking recalculations");

    let pool = db::create_pool(&config.database_url, 3)
        .await
    ...
    if redis_conn_data.get_ref().is_some() {
        let subscriber_pool = pool.clone();
        tokio::spawn(events::subscriber::run_subscriber(subscriber_pool));
    }
    ...
    HttpServer::new(move || {
        ...
        App::new()
            .wrap(cors)
            .wrap(openhack_common::metrics::MetricsMiddleware::new("leaderboard"))
