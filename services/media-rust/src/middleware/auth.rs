...
pub fn extract_auth_user(
    ...
    jwt_secret: &str,
) -> Result<AuthUser, crate::errors::AppError> {
    let auth_header = req
        .headers()
        .get("Authorization")
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| {
            crate::errors::AppError::Unauthorized("Missing Authorization header".into())
        })?;

    let token = auth_header.strip_prefix("Bearer ").ok_or_else(|| {
        crate::errors::AppError::Unauthorized("Invalid Authorization header format".into())
    })?;
...
pub fn get_auth_user(req: &actix_web::HttpRequest) -> Result<AuthUser, crate::errors::AppError> {
    ...
    let jwt_secret = req
        ...
        .ok_or_else(|| crate::errors::AppError::BadRequest("JWT secret not configured".into()))?;
...
pub fn require_admin_or_organizer(user: &AuthUser) -> Result<(), crate::errors::AppError> {
