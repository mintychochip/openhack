...
/// `JWT_SECRET`. Returns `AuthUser` on success or a `JudgingError::Unauthorized`
/// on failure (missing header, malformed token, invalid signature, expired token).
///
/// # Errors
///
/// Returns `JudgingError::Unauthorized` if:
...
pub fn extract_auth_user(
    ...
    jwt_secret: &str,
) -> Result<AuthUser, crate::errors::JudgingError> {
    let auth_header = req
        .headers()
        .get("Authorization")
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| {
            crate::errors::JudgingError::Unauthorized("Missing Authorization header".into())
        })?;

    let token = auth_header.strip_prefix("Bearer ").ok_or_else(|| {
        crate::errors::JudgingError::Unauthorized("Invalid Authorization header format".into())
    })?;
    ...
    .map_err(|e| crate::errors::JudgingError::Unauthorized(format!("Invalid token: {e}")))?;

    let Ok(user_id) = uuid::Uuid::parse_str(&token_data.claims.sub) else {
        return Err(crate::errors::JudgingError::Unauthorized(
...
/// # Errors
///
/// Returns `JudgingError::Unauthorized` if neither internal token nor valid JWT
/// is provided. Returns `JudgingError::Internal` if JWT secret is unavailable.
///
/// # Side Effects
///
...
/// - Reads the `JWT_SECRET` from app data (read-only).
pub fn get_auth_user(req: &actix_web::HttpRequest) -> Result<AuthUser, crate::errors::JudgingError> {
    ...
    let jwt_secret = req
        ...
        .ok_or_else(|| crate::errors::JudgingError::Internal("JWT secret not configured".into()))?;
...
/// Returns `JudgingError::Forbidden` if "admin" is not present.
///
/// # Errors
///
/// Returns `JudgingError::Forbidden` if the user lacks the required role.
///
/// # Side Effects
///
/// None. Pure role check with no I/O.
pub fn require_admin(user: &AuthUser) -> Result<(), crate::errors::JudgingError> {
    if user.is_admin() {
        Ok(())
    } else {
        Err(crate::errors::JudgingError::Forbidden(
...
/// Returns `JudgingError::Forbidden` if neither role is present.
///
/// # Errors
///
/// Returns `JudgingError::Forbidden` if the user lacks the required role.
...
///
/// None. Pure role check with no I/O.
pub fn require_admin_or_organizer(user: &AuthUser) -> Result<(), crate::errors::JudgingError> {
...
/// Check if the authenticated user has "judge", "admin", or "organizer" role.
///
/// # Expected Behavior
///
/// Returns `Ok(())` if the user's roles contain "judge", "admin", or "organizer".
/// Returns `JudgingError::Forbidden` if none of these roles are present.
///
/// # Errors
///
/// Returns `JudgingError::Forbidden` if the user lacks the required role.
...
///
/// None. Pure role check with no I/O.
pub fn require_judge_or_admin(user: &AuthUser) -> Result<(), crate::errors::JudgingError> {
    if user.is_judge_or_admin() {
        Ok(())
    } else {
        Err(crate::errors::JudgingError::Forbidden(
            "Judge, admin, or organizer role required".into(),
        ))
    }
}
